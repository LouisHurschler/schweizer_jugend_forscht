...
def run_modbus_forever():
    ...
    address_current = 0xF004 # Replace with the actual register address you want to read
    ...
    if client.connect():
        ...
        while True:
            # Read registers
            res_current = handle_response(client, address_current, "current", unit_id)
            res_voltage = handle_response(client, address_voltage, "voltage", unit_id)
            res_power_factor = handle_response(client, address_power_factor, "powerfactor", unit_id, len=1)

            if not (res_current and res_voltage and res_power_factor):
                continue


            curr_data = struct.pack(">4I", res_current, res_voltage, res_power_factor, int(dt.datetime.now().timestamp()) + 60 * 60)

            mqttclient.publish(topic="energy_measurements/1",payload=curr_data, qos=0, retain=False)
            time.sleep(1)
